...
/* USER CODE BEGIN 1 */
//int32_t ethernetif_get_phy_link_state(void);
/* USER CODE END 1 */
...
static user_phy_IOCtx_t UserPhyIOCtx;
...
static int32_t ETH_PHY_IO_ReadReg(uint32_t dev_addr,
                                  uint32_t reg_addr,
                                  uint32_t *reg_value);

static int32_t ETH_PHY_IO_WriteReg(uint32_t dev_addr,
                                   uint32_t reg_addr,
                                   uint32_t reg_value);
...
/* USER CODE BEGIN low_level_init Code 1 for User BSP */
  ...
  {
    UserPhyIOCtx.Init = ETH_PHY_IO_Init;
    UserPhyIOCtx.DeInit = ETH_PHY_IO_DeInit;
    UserPhyIOCtx.ReadReg = ETH_PHY_IO_ReadReg;
    ...
    else if (USER_PHY_Init(&UserPhy) != USER_PHY_STATUS_OK)
    {
        hal_eth_init_status = HAL_ERROR;
    }
  }
...
void ethernet_link_check_state(struct netif *netif)
{
    ...
    link_state = USER_PHY_GetLinkState(&UserPhy);

    /* 网线从连接变为断开 */
    if ((link_state == USER_PHY_STATUS_LINK_DOWN) &&
    ...
    /* 网线从断开变为100M全双工连接 */
    else if ((link_state == USER_PHY_STATUS_100MBITS_FULLDUPLEX) &&
             (!netif_is_link_up(netif)))
    {
...
int32_t ethernetif_get_phy_link_state(void)
{
    return USER_PHY_GetLinkState(&UserPhy);
}
/* USER CODE END 8 */
